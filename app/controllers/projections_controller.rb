class ProjectionsController < ApplicationController
  ...
  def index
    @projections = Projection.where("fulfillment_date < '2019-12-31'").order(present_date: :asc, fulfillment_date: :asc, projected_rate: :asc)
    @chart_data = []
        ...
        @projections.each do |x|
          ...
          if current_date.nil? || (current_date == x.present_date && end_date == x.fulfillment_date)
            ...
            preliminary_chart_data << {
              ...
              y: (x.projected_rate - 0.125).abs.to_f
            }
